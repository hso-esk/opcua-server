...
if [[ "$continue_flag" == y* ]]; then
	...
	cp $rootDir/$dataDir/interfaces.niki.dhcp /etc/network/interfaces

	cp $rootDir/$dataDir/6lbr.conf.niki /etc/6lbr/6lbr.conf

	cp $rootDir/$dataDir/60dhcp.niki /etc/6lbr/ifup.d/60dhcp
