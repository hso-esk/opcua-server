...
if [ ! -e "$INSTALL_DIR/${BOOST}${BOOST_VERSION}/lib/libboost_thread.so" ]; then
	...
	echo "Installing with b2 to" $INSTALL_DIR/boost-arm_${BOOST_VERSION}
    	./b2 install --prefix=${INSTALL_DIR}/../../boost-arm_${BOOST_VERSION} -link=shared toolset=gcc-arm --with-atomic --with-thread --with-chrono --with-date_time --with-system --with-test --with-filesystem --with-program_options --with-regex
	else 
	...
	echo "Installing with b2 to" ${INSTALL_DIR}/boost-x86_${BOOST_VERSION}
 	./b2 install --prefix=${INSTALL_DIR}/../../boost-x86_${BOOST_VERSION} -link=shared toolset=gcc-4.9 --with-atomic --with-thread --with-chrono --with-date_time --with-system --with-test --with-filesystem --with-program_options --with-regex
     fi
fi
