...
OpcUaLWM2MLib::OpcUaLWM2MLib(void) :
		OpcUaStackServer::ApplicationIf(), threadRun(false), ipsofileName_(""), namespaceIndex_(
				0), deviceId_(0), resourceMap_(), objectMap_(), resourceMaps_(), objectMaps_(), dbServer_(), readSensorValueCallback_(
				boost::bind(&OpcUaLWM2MLib::readSensorValue, this, _1)), readHistorySensorValueCb_(
				boost::bind(&OpcUaLWM2MLib::readHistorySensorValue, this, _1)), writeSensorValueCallback_(
				boost::bind(&OpcUaLWM2MLib::writeSensorValue, this, _1)), execSensorMethodCallback_(
				boost::bind(&OpcUaLWM2MLib::execSensorMethod, this, _1)) {
	...
	while (threadRun == true) {
		...
		while (evQueue.pop(ev)) {
			...
			case e_lwm2m_serverobserver_event_register: {
				Log(Debug, "Event device registration triggered").parameter(
						"Device name", ev.param.p_dev->getName());
			...
			case e_lwm2m_serverobserver_event_update: {
				Log(Debug, "Event device update triggered").parameter(
						"Device name", ev.param.p_dev->getName());
				/** TODO */
			...
			case e_lwm2m_serverobserver_event_deregister: {
				Log(Debug, "Event device deregistration triggered").parameter(
						"Device name", ev.param.devName);
...
bool OpcUaLWM2MLib::loadServerConfig(void) {
	...
	if (!configXmlManager.registerConfiguration(
			applicationInfo()->configFileName(), config)) {
	...
	if (!config->getConfigParameter(
			"OpcUaServerModel.IPSOConfig.IPSOConfigPath", ipsoConfigfile)) {
	...
	if (!config->getConfigParameter("OpcUaServerModel.Database.DatabaseConfig",
			dbConfigfile)) {
...
bool OpcUaLWM2MLib::decodeIPSOConfig(const std::string& configFileName) {
	...
	if (!configXmlManager.registerConfiguration(configFileName, config)) {
		Log(Debug, "Error reading IPSO config").parameter("Error message",
				configXmlManager.errorMessage()).parameter("ConfigFileName",
				configFileName);
		return false;
	}
	...
	for (auto& ipsoConfig : ipsoConfigVec) {
		...
		boost::optional<std::string> ipsoFileName = ipsoConfig.getValue(
				"<xmlattr>.File");
		if (!ipsoFileName) {
			Log(Error, "eddl path do not exist in configuration file").parameter(
					"Variable", "IPSOModel.IPSOPath.<xmlattr>.File").parameter(
					"ConfigFileName", applicationInfo()->configFileName());
			return false;
		} else {
...
bool OpcUaLWM2MLib::decodeDbConfig(const std::string& configFileName) {
	...
	if (!configXmlManager.registerConfiguration(configFileName, config)) {
		Log(Debug, "Error reading Database config").parameter("Error message",
				configXmlManager.errorMessage()).parameter("configFilename",
				configFileName);
		return false;
	}
	...
	if (!child) {
		Log(Debug, "Element missing in config file").parameter("Element",
				"DBModel").parameter("configFileName",
 ...
 * readSensorValue()
 */
void OpcUaLWM2MLib::readSensorValue(
		ApplicationReadContext* applicationReadContext) {
	Log(Debug, "OpcUaLWM2MLib::readSensorValue").parameter("id",
	...
	if (p_val && it->second.data) {
		...
		dbServer_.writeDataToDatabase(
				dbModelConfig_.databaseConfig().databaseTableName(),
				applicationReadContext->nodeId_, *it->second.data);
 ...
 */
void OpcUaLWM2MLib::readHistorySensorValue(
	...
	dbServer_.readDataFromDatabase(
			dbModelConfig_.databaseConfig().databaseTableName(),
			applicationHReadContext->nodeId_, startTime, stopTime, dataValues);

	/* create result array */
	applicationHReadContext->dataValueArray_ =
			constructSPtr<OpcUaDataValueArray>();
 ...
 * writeSensorValue()
 */
void OpcUaLWM2MLib::writeSensorValue(
		ApplicationWriteContext* applicationWriteContext) {
	Log(Debug, "OpcUaLWM2MLib::writeSensorValue").parameter("id",
			applicationWriteContext->nodeId_);
 ...
 */
void OpcUaLWM2MLib::execSensorMethod(
		ApplicationReadContext* applicationReadContext) {
...
bool OpcUaLWM2MLib::registerCallbacks(OpcUaUInt32 id) {
	...
	ServiceTransactionRegisterForwardNode::SPtr trx = constructSPtr<
	...
	req->forwardNodeSync()->readHService().setCallback(
			readHistorySensorValueCb_);
	req->forwardNodeSync()->writeService().setCallback(
			writeSensorValueCallback_);
 ...
 */
bool OpcUaLWM2MLib::createObjectDictionary(
		IPSOParser::ipsoDescriptionVec& ipsoDesc) {
	Log(Debug, "OpcUaLWM2MLib::createObjectDictionary");

	for (auto& desc : ipsoDesc) {
		if (!objectDictionary_.insert(
				objectDictionary_t::value_type(desc.objectDesc.id, &desc)).second) {
			Log(Error,
					"Object dictionary creation from IPSO descriptions failed");
			return false;
		}
	}
	return true;
...

{
	...
	for (auto& objectInfo : objectMap) {
		...
		OpcUaStackServer::BaseNodeClass::SPtr objectNode =
				informationModel()->find(objectNodeId);
		if (objectNode.get() == nullptr) {
			...
			OpcUaQualifiedName browseName(objectInfo.second.name,
					namespaceIndex_);
			...
			std::string objectDisplayName = objectInfo.second.name
					...
					+ boost::lexical_cast<std::string>(
							objectInfo.second.instanceId) + std::string(">");
			...
			OpcUaStackServer::BaseNodeClass::SPtr baseObject =
					informationModel()->find(parentObjectId);
			if (baseObject.get() != nullptr) {

				/* set reference to parent object */
				baseObject->referenceItemMap().add(
						OpcUaStackServer::ReferenceType::ReferenceType_Organizes,
						true, objectNodeId);

				objectNode->referenceItemMap().add(
						OpcUaStackServer::ReferenceType::ReferenceType_Organizes,
						false, parentObjectId);

				baseObject->referenceItemMap().add(
						OpcUaStackServer::ReferenceType::ReferenceType_HasComponent,
						true, objectNodeId);
...
bool OpcUaLWM2MLib::deleteObjectNode(std::string devName,
	...
	std::cout << devName << " object nodes successfully deleted from server"
			<< std::endl;
...
bool OpcUaLWM2MLib::createDeviceObjectNode(const LWM2MDevice* device) {
	...
	OpcUaStackServer::BaseNodeClass::SPtr baseObject = informationModel()->find(
			baseObjectId);
	...
	if (baseObject.get() != nullptr) {
		...
		baseObject->referenceItemMap().add(
				OpcUaStackServer::ReferenceType::ReferenceType_Organizes, true,
				deviceObjectNodeId);

		deviceobjectNode->referenceItemMap().add(
				OpcUaStackServer::ReferenceType::ReferenceType_Organizes, false,
				baseObjectId);

		OpcUaNodeId typeNodeId;
		typeNodeId.set(OpcUaId_FolderType);
		deviceobjectNode->referenceItemMap().add(
				OpcUaStackServer::ReferenceType::ReferenceType_HasTypeDefinition,
				true, typeNodeId);

		baseObject->referenceItemMap().add(
				OpcUaStackServer::ReferenceType::ReferenceType_HasComponent,
				true, deviceObjectNodeId);
	} else {
		return false;
	}
...
bool OpcUaLWM2MLib::createVariableNode(resourceMap_t& resourceMap) {
	...
	for (auto& varInfo : resourceMap) {
		...
		if (varInfo.second.operation == "RW" || varInfo.second.operation == "W"
			...
			OpcUaQualifiedName varbrowseName(varInfo.second.name,
					namespaceIndex_);
			...
			OpcUaStackServer::BaseNodeClass::SPtr parentObject =
					informationModel()->find(objectNodeId);
			if (parentObject.get() != nullptr) {
				...
				parentObject->referenceItemMap().add(
						OpcUaStackServer::ReferenceType::ReferenceType_HasComponent,
						true, varNodeId);
			}
...
bool OpcUaLWM2MLib::createMethodNode(resourceMap_t& resourceMap) {
	...
	for (auto& methodInfo : resourceMap) {
		...
		if (methodInfo.second.operation == "E") {
			...
			OpcUaQualifiedName methodBrowseName(methodInfo.second.name,
					namespaceIndex_);
			...
			if (parentObject.get() != nullptr) {
				...
				/* create references to object node */
				methodNode->referenceItemMap().add(
						OpcUaStackServer::ReferenceType::ReferenceType_HasComponent,
						false, objectNodeId);
...
bool OpcUaLWM2MLib::deleteResourceNodes(std::string devName,
	...
	std::cout << devName << " resource nodes successfully deleted from server"
			<< std::endl;
...
OpcUaLWM2MLib::opcUaNodeContext OpcUaLWM2MLib::createDeviceDataLWM2M(
		IPSOParser::ipsoResourceDescription opcUaNodeInfo,
	...
	ctx.data->sourceTimestamp(
			boost::posix_time::microsec_clock::universal_time());
	ctx.data->serverTimestamp(
			boost::posix_time::microsec_clock::universal_time());
	...
	if (opcUaNodeInfo.operation == "R") {
		...
		ctx.dataObject = boost::make_shared<DeviceDataLWM2M>(
				...
				opcUaNodeInfo.desc, opcUaNodeInfo.type,
		...
		ctx.dataObject = boost::make_shared<DeviceDataLWM2M>(
				opcUaNodeInfo.resource->getParent()->getParent()->getName(),
				opcUaNodeInfo.desc, opcUaNodeInfo.type, DeviceData::ACCESS_NONE,
				opcUaNodeInfo.resource);
		...
		ctx.dataObject = boost::make_shared<DeviceDataLWM2M>(
				...
				opcUaNodeInfo.desc, opcUaNodeInfo.type,
				(DeviceData::ACCESS_READ | DeviceData::ACCESS_WRITE
						| DeviceData::ACCESS_OBSERVE), opcUaNodeInfo.resource);
	}
...
int8_t OpcUaLWM2MLib::onDeviceRegister(const LWM2MDevice* p_dev) {
	...
	for (objectIterator = device->objectStart();
			objectIterator != device->objectEnd(); ++objectIterator) {
...
int8_t OpcUaLWM2MLib::onDeviceDeregister(std::string devName) {
	...
	std::cout << devName << " is deregistering from the server" << std::endl;
	...
	if (resourceMaps_.empty() && objectMaps_.empty()) {
		std::cout << "Server has no objects and resources created" << std::endl;
	}
...
void OpcUaLWM2MLib::processObserveData(const DeviceDataValue* p_val) {
	...
	for (auto& item : variables_) {
		if (item.second.dataObject->getVal() == p_val) {
			...
			/* store sensor value in database */
			dbServer_.writeDataToDatabase(
					dbModelConfig_.databaseConfig().databaseTableName(), nodeId,
					*item.second.data);
		}
	}
}
 ...
 * matchObjectId()
 */
bool OpcUaLWM2MLib::matchObjectId(LWM2MObject* lwm2mObj,
		objectDictionary_t& dictionary, objectMap_t& objectMap) {
...
bool OpcUaLWM2MLib::createLWM2MResources(objectMap_t& objectMap,
		objectDictionary_t& dictionary, resourceMap_t& resourceMap) {
			...
			for (auto& resourceItem : dictEntry.second->resourceDesc) {
				...
				if (objectItem.second.id == dictEntry.second->id) {
					if (resourceItem.operation == "R") {
						...
						LWM2MResource* resource = new LWM2MResource(
								resourceItem.resourceId, true, false, false);
						...
						resourceMap.insert(
								resourceMap_t::value_type(resourceId,
										resourceInfo));
						...
						LWM2MResource* resource = new LWM2MResource(
								resourceItem.resourceId, false, true, false);
						...
						LWM2MResource* resource = new LWM2MResource(
								resourceInfo.resourceId, false, false, true);
						...
						resourceMap.insert(
								resourceMap_t::value_type(methodId,
										resourceInfo));
						...
						LWM2MResource* resource = new LWM2MResource(
								resourceItem.resourceId, true, true, false);
 ...
 * createDataValue()
 */
OpcUaDataValue::SPtr OpcUaLWM2MLib::createDataValue(
		const DeviceDataValue* val) {
	Log(Debug, "OpcUaLWM2MLib::createObjectNode");
